...
mostIsolated(vector<double> & number)
{
	...
	isolated = val;
	dist = number[1] - val;
	for (int i = 1; i < number.size()-1;i++) { //This was as mind bending as it looks. What i'm doing is checking the shortest distance in either direction from each value and whether that distance is larger than the distance of the curent most isolated value
		val = number[i];
		if (val - number[i - 1] < number[i + 1] - val) {
			if (val - number[i - 1] > dist) {
				dist = val - number[i - 1];
				isolated = val;
			}
		}
		else if (val - number[i - 1] > number[i + 1] - val) {
			if (number[i + 1] - val > dist) {
				dist = number[i + 1] - val;
				isolated = val;
			}
		}
...
unmatched(list<string> & A, list<string> & B)
{
	...
	for (list<string>::iterator iterA = A.begin(), iterB = B.begin(); iterA != A.end()&&iterB!=B.end();) {
		wordA = *iterA;
		wordB = *iterB;
		if (wordA == wordB) {
			count--;
